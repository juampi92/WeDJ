module.exports = function(app){
	...
	var cmd = function(name , args , debug , callback ){
		...
		// Library
			case "analyze": 
				app.lib.analyze();
				callback(app.lang.get("lib.updated"));
				break;
			case "addlib":
				if ( args == null ) {
				...
				if ( ! app.lib.addFolder([ args ]) && debug ) {
					callback( (app.lang.get("lib.error.notexists")).red); 
					break;
				}
					if ( debug ) callback( app.config.getFolders() );
					else callback( app.lang.trans("lib.added",args.green) );
				break;
			case "addlibexc":
				if ( args == null ) { callback( (app.lang.get("lib.error.nullInput")).red ); break;}
				if ( ! app.lib.addExclude([ args ]) && debug ) { callback( (app.lang.get("lib.error.notexists")).red ); break; }
					if ( debug ) callback( app.config.getExcluded() );
					else callback( app.lang.trans("lib.excluded",args.green) );
				break;
			case "rmlib":
				if ( args == null ) { callback( (app.lang.get("lib.error.nullInput")).red ); break;}
				if ( ! app.lib.rmFolder( args ) ) { callback( (app.lang.get("lib.error.notexists")).red ); break;}			
					if ( debug ) callback( app.config.getFolders() );
					callback( app.lang.trans("lib.removed",(args).green) );				
				break;
			case "rmlibexc":
				if ( args == null ) { callback( (app.lang.get("lib.error.nullInput")).red ); break;}
				if ( ! app.lib.rmExclude(args) && debug ) { callback( (app.lang.get("lib.error.notexists")).red ); break; }
					if ( debug ) callback( app.config.getExcluded() );
					callback( app.lang.trans("lib.unexcluded",(args).green) );				
				break;
			case "showlibs":
				callback( app.config.getFolders() );
				break;
			case "showexc":
				callback( app.config.getExcluded() );
				break;
			case "id3Analyze":
				...
				break;	
		// Playlist
			case "queue": app.playlist.addSong(args,0); break;
			case "list": callback( app.playlist.songs ); break;
			case "song": callback( app.lib.getSong(args) ); break;
			case "current": callback( app.playlist.current ); break;
			case "state": callback( app.player.state ); break;
			case "search": app.lib.search(args , callback ); break;

		// Debug/UI
			case "lang":
				if ( args == null ) { callback( (app.lang.get("cmd.nullInput")).red );}
			...
			case "exit":
				...
				break;
			default:
				callback( app.lang.get("cmd.wrong") ); break;
		}
	}
