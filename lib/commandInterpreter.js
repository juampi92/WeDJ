module.exports = function(app){
	...
	var cmd = function(name , args , debug){
		...
		// Navigation
			case "next": 
				app.player.next(); break;
			case "stop": 
				app.player.stop(true); break;
			case "play": 
				app.player.play(); break;
			case "autopilot": 
		...
		// Library
			case "analyze": 
				app.lib.analyze();
				return app.lang.get("lib.updated");
				break;
			case "addlib":
				if ( args == null ) { return (app.lang.get("lib.error.nullInput")).red; break;}
				if ( ! app.lib.addFolder([ args ]) && debug ) { return (app.lang.get("lib.error.notexists")).red; break; }
					if ( debug ) return app.config.getFolders();
					else return app.lang.trans("lib.added",args.green);
				break;
			case "addlibexc":
				if ( args == null ) { return (app.lang.get("lib.error.nullInput")).red; break;}
				if ( ! app.lib.addExclude([ args ]) && debug ) { return (app.lang.get("lib.error.notexists")).red; break; }
					if ( debug ) return app.config.getExcluded();
					else return app.lang.trans("lib.excluded",args.green);
				break;
			case "rmlib":
				if ( args == null ) { return (app.lang.get("lib.error.nullInput")).red; break;}
				if ( ! app.lib.rmFolder( args ) ) { return (app.lang.get("lib.error.notexists")).red; break;}			
					if ( debug ) return app.config.getFolders();
					return app.lang.trans("lib.removed",(args).green);				
				break;
			case "rmlibexc":
				if ( args == null ) { return (app.lang.get("lib.error.nullInput")).red; break;}
				if ( ! app.lib.rmExclude(args) && debug ) { return (app.lang.get("lib.error.notexists")).red; break; }
					if ( debug ) return app.config.getExcluded();
					return app.lang.trans("lib.unexcluded",(args).green);				
				break;
			case "showlibs":
				return app.config.getFolders();
				break;
			case "showexc":
				return app.config.getExcluded();
				break;
			case "id3Analyze":
				return app.lang.get("lib.id3.analyze");
				app.lib.id3Analyze();
				break;	
		// Playlist
			case "queue": app.playlist.addSong(app.lib.files[args],0); break;
			case "list": return app.playlist.songs; break;
			case "song": return app.lib.getSong(args); break;
			case "current": return app.playlist.current; break;
			case "state": return app.player.state; break;
			
		// Debug/UI
			case "lang":
				if ( args == null ) { return (app.lang.get("cmd.nullInput")).red;}
				app.lang.setLang(args);
				config.setLang(args);
				return app.lang.get("cmd.langChange").green + args.grey;
				break;
			case "savecfg": 
				config.save(app.lib);
				return app.lang.get("config.saved").green;
				break;
			case "ip": 
				return app.lang.get("config.IP").green + ": ".green + (app.ip[0]+':'+app.get('port')).grey;
				break;
			case "navFolder": 
				return  app.lib.navFolder( args ); 
				break;
			case "navSongs":
				return app.lib.navSongs( args );
				break;
			case "say": 
				app.socket.broadcastChat({user:'server',msj: args });
				break;
			case "help":
			...
			case "exit":
				...
				break;
			default:
				return app.lang.get("cmd.wrong"); break;
		}
		return null;
	}
