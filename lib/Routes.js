...
module.exports = function(app) {
  ...
  app.get('/', function(req, res) {
    res.render('index', {
      ...
      admin: app.users.isAdminFromIP(req.connection.remoteAddress),
  ...
  app.get('/lang', function(req,res){
    res.send(JSON.stringify(app.lang.get("view")));
  });

  // Auth
  app.get('/auth', function(req, res) {
    if (!app.users.isLogged(req.connection.remoteAddress))
      res.send('{"status":"error","type":1, "msj":"' + app.lang.get("api.notLogged") + '","admin":"' + app.users.isAdminFromIP(req.connection.remoteAddress) + '"}');
    else {
      var usr = app.users.getUserfromIP(req.connection.remoteAddress);
      res.send('{"status":"ok","msj":"' + app.lang.get("api.logged") + '","id":' + usr.id + ',"name":"' + usr.name + '","admin":"' + app.users.isAdminFromIP(req.connection.remoteAddress) + '"}');
    }
  ...
  app.post('/auth', function(req, res) {
    ...
    res.send('{"status":"ok", "msj":"' + app.lang.trans("api.loggedAs", req.body.name) + '", "id":' + id + ' , "admin":"' + app.users.isAdminFromIP(req.connection.remoteAddress) + '"}');
  });
  ...
  app.post('/api/admin', function(req, res) {
    if (!app.users.isAdminFromIP(req.connection.remoteAddress)) {
  ...
  app.get('/server', function(req, res) {
    if (!app.users.isOwner(req.connection.remoteAddress))
