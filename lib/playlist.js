module.exports = function(users){
	...
	Playlist.addSong = function( track , user ) {
		...
		console.log(" + ".bold.cyan + (users.getUser(user).name).grey + " agregó " + (track.path).green);
	...
	Playlist.voteNext = function(usr){
		...
		console.log(this.votenext.length + " - " + usrs.length + " - " + Math.ceil((usrs.length)/2));

		if ( this.votenext.length-1 >= Math.ceil((usrs.length)/2) ) {
