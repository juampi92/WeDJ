...
Library.addFolder = function(folderArray){
	for (var i = 0; i < folderArray.length; i++)
		this.folders.push(folderArray[i]);
}

Library.checkAnalyze = function(){
	if ( ! fs.existsSync("./settings/paths.json") || ! fs.existsSync("./settings/library.json") ) {
		console.log( "WARNING:".red + " Es necesario analizar la libreria. No hay canciones disponibles.\nUso: \n\taddlib <absolutepath>\n\tanalyze");
		return true;
	}
	return false;
}
...
Library.exclude = function(folderArray) {
	this.excluded.concat(folderArray);
};
