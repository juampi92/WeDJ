...
module.exports = function(app){
	...
	Library.load = function(){
		...
		// Instalar la db
		app.db.paths = new Datastore({ filename: './settings/db/paths.db', autoload: true });
		app.db.library = new Datastore({ filename: './settings/db/library.db', autoload: true });

		app.db.paths.ensureIndex({ fieldName: 'parent_id' });
		app.db.library.ensureIndex({ fieldName: 'parent_id' });
		app.db.library.ensureIndex({ fieldName: 'id' });

		app.db.library.find({}, function (err, docs) { self.canciones = docs.length; });
	...
	Library.analyze = function () {
		...
		var folders = app.config.getFolders();
	...
	Library.addFolder = function(folderArray){
		...
		app.config.addFolder(folderArray);
		return true;
	}
	...
	Library.navFolder = function(parent , callback ){
		app.db.paths.find({parent_id: parent}).sort({ name: 1}).exec(function (err, sub_paths) {
	...
	Library.navSongs = function(folder , callback ){
		app.db.library.find({parent_id: folder}).sort({ path: 1}).exec(function (err, songs) {
			if ( err ) callback(null);
			else callback(songs);
		});
	}
	...
	Library.search = function( query , callback ){
		...
		app.db.library.find({ path: { $regex: new RegExp('(' + query + ')', 'i') } }).sort({ name: 1}).exec(function (err, songs) {
			if ( err ) { console.log(lang.get("playlist.notExists")); callback(null); } 
			else {
				var folders = {};
	...
	Library.getSong = function(id , callback ){
		app.db.library.findOne({ _id: id }, function (err, song) {
			if ( err ) { console.log(lang.get("playlist.notExists")); callback(null); } 
			else callback(song);
		});
	}
